...
def build_manual_void_preview(
    ...
    *,
    ...
    return {
        "target_event": dict(target),
        "void_event": event.to_dict(),
    }
...
def build_manual_repair_preview(
    ...
    *,
    ...
    repair_event = TradeEvent(
        event_id=repair_event_id,
        source_type="manual_trade_event",
        source_name="cli_trade_event_repair",
        broker=str(core.get("broker") or ""),
        account=str(core.get("account") or ""),
        symbol=str(core.get("symbol") or ""),
        option_type=str(core.get("option_type") or ""),
        side=str(core.get("side") or ""),
        position_effect=str(core.get("position_effect") or ""),
        contracts=int(core.get("contracts") or 0),
        price=float(core.get("price") or 0.0),
        strike=(float(core["strike"]) if core.get("strike") is not None else None),
        multiplier=(int(core["multiplier"]) if core.get("multiplier") is not None else None),
        expiration_ymd=(str(core.get("expiration_ymd") or "").strip() or None),
        currency=str(core.get("currency") or ""),
        trade_time_ms=int(core.get("trade_time_ms") or now_ms()),
        order_id=(str(core.get("order_id") or "").strip() or None),
        multiplier_source=(str(core.get("multiplier_source") or "").strip() or None),
        raw_payload=core_raw_payload,
    )
    void_event = TradeEvent(
        ...
        raw_payload={
            ...
            "void_target_event_id": str(target_event_id),
            "void_reason": str(repair_reason or "manual_repair"),
...
def persist_manual_repair_event(
    ...
    *,
    ...
    void_event = TradeEvent(**preview["void_event"])
    repair_event = TradeEvent(**preview["repair_event"])
    ...
    result = _with_sqlite_repo_transaction(repo, _run)
