...
from domain.domain.ledger.position_fields import normalize_account, normalize_broker, normalize_option_type, normalize_side, now_ms
...
from src.application.positions.sync_config import (
...
def normalize_local_fields(fields: dict[str, Any]) -> dict[str, Any]:
    return normalize_position_lot_fields(fields)
...
def build_sync_run_result(
    *,
    ...
    result: dict[str, Any] = {
        ...
        "schema_version": "1.0",
        "source_of_truth": "canonical_position_lots",
        "mirror_kind": "feishu_option_positions",
        "mirror_only": True,
        "mode": str(mode),
        "status": status,
...
def sync_option_positions(
    *,
    ...
    table_ref = position_lot_mirror_table_ref(data_config)
    local_records = list_position_lot_sync_snapshots(repo, base=base)
        ...
        if not feishu_record_id:
            ...
            if apply_mode and not writes_enabled:
                ...
                try:
                    ...
                    sync_patch = build_position_lot_sync_metadata_patch(
                        feishu_record_id=feishu_record_id,
                        sync_hash=outgoing_hash,
                        synced_at_ms=synced_at_ms,
                    )
                    record_position_lot_sync_metadata(repo, candidate.record_id, sync_patch)
...
def main(argv: list[str] | None = None) -> None:
    ...
    data_config, repo = open_position_ledger_from_data_config(base=base, data_config=data_config_ref)
    ...
    if args.verbose:
        ...
        local_records = list_position_lot_sync_snapshots(repo, base=state_base)
        candidates = select_candidates(
            local_records,
            only_record_id=args.only_record_id,
            only_open=bool(args.only_open),
