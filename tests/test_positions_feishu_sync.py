...
def test_sync_script_dry_run_reports_create(monkeypatch, tmp_path: Path, capsys) -> None:
    import src.application.positions.feishu_sync as sync_mod
...
def test_sync_script_apply_updates_existing_feishu_row_and_persists_metadata(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    repo.update_position_lot_sync_metadata(
        lot["record_id"],
        PositionLotSyncMetadataPatch(feishu_record_id="rec_existing"),
    )
...
def test_sync_script_apply_persists_last_run_and_receipt_state(monkeypatch, tmp_path: Path, capsys) -> None:
    ...
    assert state["source_of_truth"] == "canonical_position_lots"
    assert state["mirror_kind"] == "feishu_option_positions"
    assert state["mirror_only"] is True
...
def test_sync_dry_run_does_not_prune_remote_when_scan_is_limited(monkeypatch, tmp_path: Path) -> None:
    ...
    second = ledger_manual_trades.persist_manual_open_event(
        repo,
        OpenPositionCommand(
            broker="富途",
            account="lx",
...
def test_sync_apply_deletes_remote_orphan_when_enabled(monkeypatch, tmp_path: Path) -> None:
    ...
    ledger_interventions.persist_manual_void_event(repo, target_event_id=str(open_result["event_id"]), void_reason="cleanup", as_of_ms=2000)
...
def test_sync_skips_unchanged_payload_even_when_last_synced_at_changes(monkeypatch, tmp_path: Path) -> None:
    ...
    repo.update_position_lot_sync_metadata(
        ...
        build_position_lot_sync_metadata_patch(
            ...
            sync_hash=sync_mod.sync_payload_hash(payload),
            synced_at_ms=1000,
        ),
    )
...
def test_sync_apply_update_sends_numeric_payload_types(monkeypatch, tmp_path: Path) -> None:
    ...
    repo.replace_position_lots([PositionLotRecord(record_id=lot["record_id"], fields=patched_fields)])
