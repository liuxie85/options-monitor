...
def test_execute_multi_account_delivery_collects_mixed_success_and_unconfirmed(fake_runlog_factory) -> None:
    ...
    out = mod.execute_multi_account_delivery(
        delivery_plan=_plan({"lx": "msg-lx", "sy": "msg-sy"}),
        run_id="run-1",
...
def test_execute_multi_account_delivery_collects_all_failures(fake_runlog_factory) -> None:
    mod = importlib.import_module("src.application.scheduled_notification")
    normalize = importlib.import_module("domain.domain").normalize_notify_subprocess_output
    failure_codes: list[str] = []
...
def test_execute_multi_account_delivery_preserves_success_order(fake_runlog_factory) -> None:
    mod = importlib.import_module("src.application.scheduled_notification")
    normalize = importlib.import_module("domain.domain").normalize_notify_subprocess_output
