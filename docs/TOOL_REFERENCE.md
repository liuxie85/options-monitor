...
自动升级是显式 opt-in：`./om service render --include-auto-upgrade ...` 会额外生成每天北京时间 06:10 的 upgrade timer。升级命令默认 dry-run；只有 `./om update apply --confirm` 才会下载 release tag、切换 `current` symlink 并按 profile 重启长期服务。`./om update rollback` 同样默认 dry-run。旧的 `./om service upgrade*` / `./om service rollback` 入口仍保留兼容。
