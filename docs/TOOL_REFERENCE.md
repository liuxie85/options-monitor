...
它不是 `om-agent` manifest 里的工具，也不是 shell bridge。`inbound feishu` 只解析 Feishu 事件 payload，然后进入同一条 sender allowlist、message_id 幂等、SQLite audit 和 pure-read 白名单路径。`inbound feishu-gateway` 是长驻 Feishu App event callback receiver：负责 Feishu 签名/verification token 校验、只读 inbound 调用和 Feishu 自动回复。完整边界见 [INBOUND_CONTROL.md](INBOUND_CONTROL.md)。
...
```bash
./om service render --target systemd --runtime-root /var/lib/options-monitor --env-file /etc/options-monitor/options-monitor.env --markets us hk --accounts lx sy --include-feishu-gateway --output-dir /tmp/options-monitor-service
...
`service render` 只生成 service/timer/plist/profile 文件和安装命令，不会自动安装或启动。systemd 的 `--env-file` 会写入 `EnvironmentFile=...`，用于加载服务器本地的 Feishu 凭证环境变量。systemd unit 始终写入 `OM_RUNTIME_ROOT`；只有显式传 `--deploy-user` 或设置 `OM_DEPLOY_USER` / `DEPLOY_USER` 时，才会写入 `User=<deploy_user>` 和默认 `HOME=/home/<deploy_user>`。HOME 不在默认位置时用 `--deploy-home` 覆盖。启用 `--include-feishu-gateway` 时会额外生成 `options-monitor-feishu-gateway.service`，默认监听 `127.0.0.1:8765/feishu/events`，建议由 Nginx/Caddy/Cloudflare Tunnel 负责 HTTPS。启用 `--include-auto-upgrade` 时，`--repo-root` 会保留传入的 symlink 路径，默认 config path 会使用 runtime root 下的 `config.us.json` / `config.hk.json`，避免生产配置随 release 目录漂移。升级切换 symlink 前会迁移上一 release 的 `configs/user*.json`，并用新 release 重建/校验 runtime config；缺少 market user overlay 时会 fail fast 并写 remediation。`service preflight` 是只读部署前检查；`service repair-output` 默认 dry-run，只有带 `--confirm` 才会迁移真实目录并创建 `output` symlink。
...
自动升级是显式 opt-in：`./om service render --include-auto-upgrade ...` 会额外生成每天北京时间 06:10 的 upgrade timer。升级命令默认 dry-run；只有 `./om update apply --confirm` 才会下载 release tag、准备新 release `.venv`、安装 runtime/server 依赖、校验新 release、切换 `current` symlink 并按 profile 重启长期服务。systemd 使用非 root `User=` 运行时，profile 会使用 `sudo -n systemctl restart options-monitor-trade-intake.service`，需要部署用户具备对应 NOPASSWD sudoers；授权缺失时 upgrade status 会记录 remediation。`./om update rollback` 同样默认 dry-run。旧的 `./om service upgrade*` / `./om service rollback` 入口仍保留兼容。
