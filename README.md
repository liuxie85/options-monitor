...
Agent 可用 `version_update` 预览或更新本地 `VERSION`；默认只预览，显式 `apply=true` 才写入。
...
不要把 `{"bump":"patch","apply":true}` 直接放进固定频率定时任务；它每次执行都会把本地
`VERSION` 递增一次。真正需要自动写入时，应由上游发布流程传入明确目标版本，例如
`{"version":"1.2.3","apply":true}`。这个工具只改本地 `VERSION`，不会创建 tag、commit、
push 或部署。
...
| 交易监听 / 自动入账 | `python3 scripts/auto_trade_intake.py --config config.us.json --mode apply` | 长驻进程，由 supervisor / service 管理 | 监听 OpenD 成交推送，写本地 `option_positions` 和 intake state |
| 持仓镜像同步 | `python3 scripts/sync_option_positions_to_feishu.py --apply` | 低频定时或人工触发 | 写 Feishu 镜像表，并更新本地 sync metadata |
...
```bash
python3 scripts/auto_trade_intake.py --config config.us.json --once
```
...
版本更新同理，普通定时任务只做检查或 dry-run；不要把 `{"bump":"patch","apply":true}` 放进固定频率任务。
