...
- `option_positions` bootstrap
- `option_positions` 远端镜像
- 飞书通知
...
| 任务 | 推荐入口 | 运行方式 | 主要副作用 |
|---|---|---|---|
| 期权监控 / 扫描通知 | `./om run tick-cron --market hk --accounts lx sy --timeout 600` / `./om run tick-cron --market us --accounts lx sy --timeout 600` | cron 每 10 分钟唤醒，代码内判断业务窗口 | 写本地报告和运行状态，可能过期自动平仓，并按 receipt 配置发送回执/通知 |
...
| Option positions Feishu 镜像 | `./om option-positions sync-feishu --config config.us.json --apply` | 低频定时或人工触发 | 写本地 sync metadata 与 Feishu，并按 receipt 配置发送任务级回执 |
...
| `./om option-positions sync-feishu --apply` | 是 | 是 | 是，默认发送任务级同步回执 |
| `./om option-positions sync-feishu --apply --no-send` | 是 | 是 | 否 |
...
| Feishu 同步没有回执 | `runtime_status` 的 `option_positions_feishu_sync`、`option_positions.sync_to_feishu.receipt.enabled`、通知 route；重跑时还要看 `receipt_key` 是否已确认发送 |
| 平仓建议异常 | `prepare_close_advice_inputs`、本地 `option_positions`、required data |
| Feishu 同步异常 | `option_positions.sync_to_feishu.enabled`、`runtime_status.option_positions_feishu_sync`、`sync-feishu --dry-run`、`portfolio.data_config` |
...
- [docs/GETTING_STARTED.md](docs/GETTING_STARTED.md)：Agent 快速开始
